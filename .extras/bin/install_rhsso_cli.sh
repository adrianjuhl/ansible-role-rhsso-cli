...
Usage: $(basename "${BASH_SOURCE[0]}")
           [--install_bin_dir=<dir>]
           [--rhsso_cli_executable_name=<name>]
...
Available options:
  --install_bin_dir=<dir>
      The directory where rhsso_cli is to be installed. Defaults to "/usr/local/bin".
  --rhsso_cli_executable_name=<name>
      The name that the executable is to be named. Defaults to "rhsso_cli".
  --requires_become=<true|false>
...
install_rhsso_cli()
{
  ...
  ANSIBLE_ROLES_PATH=${HOME}/.ansible/roles/ ansible-galaxy install --role-file=${THIS_SCRIPT_DIRECTORY}/../.ansible/roles/requirements_rhsso_cli.yml --force
  ...
  ansible-playbook ${ANSIBLE_CHECK_MODE_ARGUMENT} ${ANSIBLE_DIFF_MODE_ARGUMENT} ${ASK_BECOME_PASS_OPTION} -v \
    --inventory="localhost," \
    --connection=local \
    --extra-vars="adrianjuhl__rhsso_cli__install_bin_dir=${INSTALL_BIN_DIR}" \
    --extra-vars="adrianjuhl__rhsso_cli__rhsso_cli_executable_name=${RHSSO_CLI_EXECUTABLE_NAME}" \
    --extra-vars="local_playbook__install_rhsso_cli__requires_become=${REQUIRES_BECOME}" \
    ${THIS_SCRIPT_DIRECTORY}/../.ansible/playbooks/install_rhsso_cli.yml
}
...
parse_script_params()
{
  ...
  RHSSO_CLI_EXECUTABLE_NAME="rhsso_cli"
